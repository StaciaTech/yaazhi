      ...
      <section className="section-padding pt-8">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {videos.map((video, index) => <div key={video.title} className="group relative aspect-video rounded-2xl overflow-hidden shadow-card cursor-pointer opacity-0 animate-fade-up" style={{
      ...
      <section className="section-padding text-black bg-black">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 md:gap-6">
            {performancePhotos.map((photo, index) => <div key={index} className={`group relative rounded-2xl overflow-hidden shadow-card cursor-pointer opacity-0 animate-fade-up ${index === 0 || index === 4 ? "row-span-2" : ""}`} style={{
            animationDelay: `${(index + 1) * 80}ms`,
            animationFillMode: "forwards"
          }}>
                <img src={photo.src} alt={photo.alt} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" style={{
              minHeight: index === 0 || index === 4 ? "400px" : "200px"
            }} />
      ...
      <section className="section-padding">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
            {teamPhotos.map((photo, index) => <div key={index} className="group relative aspect-square rounded-2xl overflow-hidden shadow-card opacity-0 animate-fade-up" style={{
            animationDelay: `${(index + 1) * 100}ms`,
            animationFillMode: "forwards"
          }}>
